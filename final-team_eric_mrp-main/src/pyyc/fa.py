...
def flatten_expr(n, var=None):
    ...
    if isinstance(n, ast.Constant):
        ...
        if isinstance(n.value, ast.BinOp) or isTypeCheckFunction(n.value):
            ...
            if not var:
                return "inject_int(" + str(ret[0]) + ")", ret[1]
        ...
        ret, ret_code = flatten_expr(n.value)
        return "project_int(" + str(ret) + ")", ret_code

    elif isinstance(n, explicate.UnboxBig):
        ret, ret_code = flatten_expr(n.value)
        return "project_big(" + str(ret) + ")", ret_code

    elif isinstance(n, explicate.UnboxBool):
        ret, ret_code = flatten_expr(n.value)
        return "project_bool(" + str(ret) + ")", ret_code
        ...
        if isinstance(n.op, ast.And):  # and not isTypeCheckFunction(n.values[0]):
            ...
            for x in range(len(var1) - 1):
                ...
                if isinstance(n.values[x], ast.List) or isinstance(n.values[x], ast.Dict):
                    temp3 = get_new_temp()
                    string = (temp2 + " = " + var1[
                        x] + (("\n" + temp3 + " = is_true(" + temp2 + ")") if "is_" not in var1[
        ...
        for arg in n.args:
            ...
            if flat_arg.find("=") == -1 or "\n" in flat_arg[0:flat_arg.find("=")]:
        ...
        if isinstance(n.func, ast.Name) and "print" in n.func.id:
...
def flattenAST(n):
    ...
    if isinstance(n, ast.Module):
        ...
        for x in n.body:
            ...
            code += temp
        print(code)
        return remove_nested_calls(remove_nested_calls(remove_empty_lines(code)))
        ...
        if isinstance(n.value, explicate.InjectInt) and not rhs_code and rhs.find(targets + " = ") != -1:
            return "" + rhs + "\n"
