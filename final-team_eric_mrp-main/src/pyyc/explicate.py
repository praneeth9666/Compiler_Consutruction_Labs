...
class ExplicateAdd(ast.NodeTransformer):
    ...
    def visit_Assign(self, node):
        ...
        if isinstance(target, ast.Subscript):
            sub_value = self.visit(target.value)
            if isinstance(target.slice, ast.Constant):
                
                if isinstance(target.slice.value, int):
                    index = InjectInt(value=target.slice.value)
                elif isinstance(target.slice.value, bool):
                    index = InjectBool(value=target.slice.value)
                else:
                    index = InjectBig(value=target.slice.value)
            else:
                index = self.visit(target.slice)
            target = ast.Subscript(value=sub_value, slice=index, ctx=target.ctx)
        elif isinstance(target, ast.Name) and isinstance(value, (InjectInt, InjectBig, InjectBool)):
            self.cache[target.id] = value
    ...
    def visit_BinOp(self, node):
        left_key = node.left.id if isinstance(node.left, ast.Name) else None
        right_key = node.right.id if isinstance(node.right, ast.Name) else None
        if isinstance(node.op, ast.Add):
            if left_key in self.cache and right_key in self.cache:
                left = self.cache[left_key]
                right = self.cache[right_key]
                if isinstance(left, InjectInt) and isinstance(right, InjectInt):
                    return InjectInt(value=ast.BinOp(left=UnboxInt(value=left), op=node.op, right=UnboxInt(value=right)))
                elif isinstance(left, InjectBool) and isinstance(right, InjectBool):
                    return InjectBool(value=ast.BinOp(left=UnboxBool(value=left), op=node.op, right=UnboxBool(value=right)))
                elif isinstance(left, InjectInt) and isinstance(right, InjectBool):
                    return InjectInt(value=ast.BinOp(left=UnboxInt(value=left), op=node.op, right=UnboxBool(value=right)))
                elif isinstance(left, InjectBool) and isinstance(right, InjectInt):
                    return InjectInt(value=ast.BinOp(left=UnboxBool(value=left), op=node.op, right=UnboxInt(value=right)))        
                elif isinstance(left, InjectBig) and isinstance(right, InjectBig):
                    return InjectBig(value=BigAdd(left=UnboxBig(value=left), right=UnboxBig(value=right)))
                else:
                    return CallError()    
            else:
