...
def isTypeCheckFunction(n):
    return (isinstance(n, explicate.CheckIntTag) or isinstance(n, explicate.CheckBigTag) or isinstance(n,
            ...
            or isinstance(n, explicate.UnboxBool) or isinstance(n, explicate.UnboxInt) or isinstance(n,
                                                                                                     explicate.UnboxBig)
            or isinstance(n, explicate.BigAdd) or isinstance(n, explicate.CallError) or isinstance(n, explicate.GetFrees)

            or isinstance(n, explicate.GetFunc) or isinstance(n, explicate.CheckFunTag))
...
def flatten_expr(n, var=None):
    ...
    if isinstance(n, ast.IfExp):
        ...
        temp3 = get_new_temp()
        code = test_code
        code += ((temp3 + " = is_true(" + test + ")\n") if "is_" not in test else "")
        code += "if " + (temp3 if "is_" not in test else test) + ":\n\x09"
        if_body = body_code + temp + " = " + body + "\n"
        code += if_body.replace("\n", "\n\x09")[0:-1]
        code += "else:\n\x09"
        code += else_code.replace("\n", "\n\x09") + (temp + " = " + else_body.replace("\n", "\n\x09").replace("temp_replace_me", temp) if "Type Checking Error" not in else_code else "") + "\n"
        code = code[0:-1] if code[-1] == "\x09" else code
        return temp, code
    ...
    if isinstance(n, ast.Constant):
    ...
    elif isinstance(n, explicate.CheckFunTag):
        return "inject_int(is_function(" + str(flatten_expr(n.value)[0]) + "))", ""
        ...
        if isinstance(n.value, ast.Call):
            temp = get_new_temp()
            ret = flatten_expr(n.value, temp)
            return temp, ret[1] + temp + " = " + str(ret[0]) + "\n" + temp + " = inject_big(" + temp + ")\n"
    ...
    elif isinstance(n, explicate.IsTrue):
        return "is_true(" + str(flatten_expr(n.value)[0]) + ")", ""
    ...
    elif isinstance(n, explicate.GetFunc):
        return "get_fun_ptr(" + str(flatten_expr(n.value)[0]) + ")", ""

    elif isinstance(n, explicate.GetFrees):
        return "get_free_vars(" + str(flatten_expr(n.value)[0]) + ")", ""
        ...
        if isinstance(n.func, ast.Name) and n.func.id == "print_any":
            ...
            res = flattenAST(n.func) if not isinstance(n.func, ast.Call) and not isinstance(n.func, explicate.Let) else flatten_expr(n.func)
            temp = get_new_temp()
            if isinstance(res, tuple):
                code = args_code + preamble + res[1] + temp + " = " + res[0] + "(" + args_str + ")\n"
            else:

                code = args_code + preamble + temp + " = " + res + "(" + args_str + ")\n"
            return temp, code
...
def flattenAST(n):
    ...
    if isinstance(n, ast.Module):
        return remove_nested_calls(remove_empty_lines("".join([flattenAST(x) for x in n.body])))
        ...
        for x in n.body:
            code += flattenAST(x)
        args = flattenAST(n.args)
        return "def " + n.name + "(" + args + "):\n" + ("\x09" if code[0] != "\x09" else "") + code.replace("\n", "\n\x09")[0:-1]
        ...
        args = ""
        for x in n.posonlyargs:
            args += flattenAST(x) + ", "
        for x in n.args:
            args += flattenAST(x) + ", "
        args += (flattenAST(n.vararg) + ", " if n.vararg is not None else "")
        for x in n.kwonlyargs:
            args += flattenAST(x) + ", "
        for x in n.kw_defaults:
            args += flattenAST(x) + ", "
        args += (flattenAST(n.kwarg) + ", " if n.kwarg is not None else "")
        for x in n.defaults:
            args += flattenAST(x) + ", "
        args = args[0:-2]
        return args
        ...
        res = flatten_expr(n.value)
        temp2 = get_new_temp()
        return res[1] + temp2 + " = " + res[0] + "\nreturn " + temp2 + "\n"
...
def remove_nested_calls(code):
    lines = code.split("\n")
    for x in range(len(lines)):
        line = lines[x]
        outer_fun = line.find("(")
        if outer_fun != -1:
            inner_fun = line.find("(", outer_fun + 1)
            if inner_fun != -1:
                inner_call = line[outer_fun + 1: line.find(")") + 1]
                temp3 = get_new_temp()
                lines[x] = line.replace(inner_call, temp3)
                new_line = ("\x09" * line.count("\x09")) + temp3 + " = " + inner_call
                lines.insert(x, new_line)
    return "\n".join(lines)
