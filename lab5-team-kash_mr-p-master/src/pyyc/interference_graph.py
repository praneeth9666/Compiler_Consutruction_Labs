...
def build_interference_graph(cfg,var_lst):
    ...
    for var in var_lst:
        #var= var.replace(" =","")
        if("%" not in var and "error" not in var and "main" not in var):
            interference_graph.setdefault(var,set())
    callee_saved_regs = {'eax', 'ecx', 'edx'}

    def add_interference(var1, var2):
        if "%" not in var1 and "%" not in var2 and "error" not in var1 and "error" not in var2 and "main" not in var1 and "main" not in var2:
            #print(var1,var2)
