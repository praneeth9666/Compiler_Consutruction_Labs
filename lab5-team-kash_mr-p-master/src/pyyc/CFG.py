...
class CFG:
    ...
    def set_function_name(self, label):
        self.label = label
...
def create_CFGs(ir, fun_delimiter):
    stack = [""]
    irs = []
    cfgs = []
    splits = ir.split("\n")
    for x in splits:
        if fun_delimiter in x and ":" in x: # Check if the function delimiter is not a control flow instruction
            stack.append("")
            stack[-1] += x + "\n"
            continue
        if "ret" in x:
    ...
    if stack:
        irs.append(stack[-1])
    for x in irs:
        cfgs.append(createCFG(x))
        if x.strip() != "":
            cfgs[-1].set_function_name(x.split()[0])
        else:
            cfgs.pop()
    return cfgs
